#rev. 0.220428
#rev. 0.220430
#rev. 0.220708  增加了对于.srt等字幕文件的匹配
#rev. 0.220710  由于分盘特征-cdn和内置字幕-c有相同特征，增加了正则表达进行区分；增加了对于相同分辨率的判断
        ...
        for olddirname in w_name:
            if os.path.isdir (olddirname): # 判断是否为目录名
                ...
                for subname in s_names:
                    ...
                    if res != "" and res not in resl:   #增加了对于相同分辨率的判断，如果几个视频分辨率相同，在resl里只不用重复添加
                        resl.append(res)
                    if re.search("-c(?!d)|.srt|.ass|.saa",subname,re.I) == None:  ## 判断文件名是否有-C或者\.srt\.ass\.saa等字幕文件,search()方法用于在整个字符串中搜索第一个匹配的值,如果匹配成功,则返回一个Match对象,否则返回None.
                        match += 0              ##正则表达式(?!)为反向匹配，例如上面 -c(?!d) 无法匹配到 -cd；另外 (?=)为正向匹配，例如 -c(?=d) 可以匹配到 -c 
                    else:
                        match += 1  #文件名有一个带-C或者带有字幕文件，就可认定为带中文字幕 2022/07/08
                resl_str = ",".join(resl)
